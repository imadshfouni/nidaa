...
const PortalScene = lazy(() =>
  import('@/components/three/PortalScene').then((m) => ({ default: m.PortalScene })),
)
...
export function HeroSection() {
  ...
  const portalY = useTransform(scrollYProgress, [0, 1], [0, reduced ? 0 : 80])
  const textY = useTransform(scrollYProgress, [0, 1], [0, reduced ? 0 : 40])
  const opacity = useTransform(scrollYProgress, [0, 0.8], [1, reduced ? 1 : 0.3])
  ...
  return (
    ...
    >
      ...
      <div className="container-premium relative z-10 grid items-center gap-16 px-5 pt-28 pb-20 lg:grid-cols-2 lg:gap-20 lg:px-14 lg:pt-32">
        <motion.div style={{ y: textY, opacity }} className="max-w-2xl">
          ...
          <motion.h1
            className="font-display text-[2.75rem] font-bold leading-[1.02] tracking-[-0.03em] sm:text-6xl lg:text-[4.5rem] xl:text-[5rem]"
        ...
        <motion.div style={{ y: portalY }} className="relative">
          <div className="absolute inset-0 rounded-full bg-gradient-to-br from-blue/15 via-violet/10 to-transparent blur-3xl" />
          <Suspense fallback={<div className="h-[380px] animate-pulse rounded-3xl bg-white/5 sm:h-[460px] lg:h-[560px]" />}>
            <PortalScene />
          </Suspense>
        </motion.div>
      </div>
